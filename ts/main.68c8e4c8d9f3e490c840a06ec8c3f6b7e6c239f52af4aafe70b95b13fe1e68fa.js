(() => {
  // ns-hugo-imp:E:\blog\hugostack\themes\hugo-theme-stack\assets\ts\gallery.ts
  ...
  // ns-hugo-imp:E:\blog\hugostack\themes\hugo-theme-stack\assets\ts\color.ts
  var colorsCache = {};
  ...
  // ns-hugo-imp:E:\blog\hugostack\themes\hugo-theme-stack\assets\ts\menu.ts
  ...
  // ns-hugo-imp:E:\blog\hugostack\themes\hugo-theme-stack\assets\ts\colorScheme.ts
  ...
  // ns-hugo-imp:E:\blog\hugostack\themes\hugo-theme-stack\assets\ts\scrollspy.ts
  ...
  // ns-hugo-imp:E:\blog\hugostack\themes\hugo-theme-stack\assets\ts\smoothAnchors.ts
  ...
  var Stack = {
    swup: null,
    initSwup: () => {
      if (typeof window.Swup === "undefined" || typeof window.SwupSlideTheme === "undefined" || typeof window.SwupPreloadPlugin === "undefined" || typeof window.SwupProgressPlugin === "undefined") {
        setTimeout(Stack.initSwup, 100);
        return;
      }
      try {
        Stack.swup = new window.Swup({
        ...
        if (Stack.swup && Stack.swup.on) {
          Stack.swup.on("contentReplaced", () => {
            Stack.reinitializeComponents();
            Stack.updateNavigationMenu();
          });
          Stack.swup.on("pageView", () => {
            Stack.updateNavigationMenu();
          });
          Stack.swup.on("clickLink", () => {
    ...
    reinitializeComponents: () => {
      ...
      if (articleTile) {
        Stack.initArticleTile(articleTile);
      }
      Stack.initCodeCopyButtons();
    },
    init: () => {
      Stack.initSwup();
      menu_default();
    ...
    updateNavigationMenu: () => {
      ...
      const menuItems = document.querySelectorAll("#main-menu > li:not(.menu-bottom-section)");
      ...
      menuItems.forEach((item, index) => {
        ...
        console.log(`Menu item ${index}: link=${linkUrl}, path=${linkPath}`);
        const isExactMatch = currentUrl === linkUrl;
        const isPathMatch = currentPath === linkPath;
        const isSubPathMatch = currentPath !== "/" && linkPath !== "/" && currentPath.startsWith(linkPath) && currentPath !== linkPath;
        const isCurrent = isExactMatch || isPathMatch || isSubPathMatch;
        console.log(`  - Exact match: ${isExactMatch}`);
        console.log(`  - Path match: ${isPathMatch}`);
        console.log(`  - Sub-path match: ${isSubPathMatch}`);
        console.log(`  - Should be current: ${isCurrent}`);
        if (isCurrent) {
          item.classList.add("current");
          console.log(`  - ADDED current class to item ${index}`);
        } else {
          item.classList.remove("current");
          console.log(`  - REMOVED current class from item ${index}`);
        }
      });
    }
  };
